...
class Attention(nn.Module):
    ...
    def forward(
        ...
        # Backward compatibility if caching is not enabled or inputs are missing
        ...
        if is_initial or is_prompt_up:
            ...
            # Apply RoPE directly with apply_rope helper
        ...
        if is_initial or is_resp_full:
            ...
            v_r_all = self.v_proj(response_x).view(B, T_r, KVH, HD).transpose(1, 2) # [B, KVH, T_r, HD]
            v_r_all_seq = v_r_all.transpose(1, 2) # [B, T_r, KVH, HD]
            
            tracker = TokenTracker(update_ratio=update_ratio)
            cached_v_response = cache.get_cached_v_response()
            indices_resp = tracker.verify(v_r_all_seq, cached_v_response) # [B, S]
            ...
            if indices_resp.shape[1] == 0:
                ...
                # Inline RoPE for scattered tokens (requires batch expansion)
                ...
                # Assemble local response K/V to use for Attention calculation
        ...
        # --- 4. COMPUTE ATTENTION & UPDATE CACHES ---
        ...
        if needs_resp_attn == 'full':
            ...
            # Update cache which implicitly performs the scatter operation
            cache.update_response_partial(k_partial_rope, v_r_all_seq, attn_out_r_partial, indices_resp)
            
            # Fetch the completed scattered response attention output
